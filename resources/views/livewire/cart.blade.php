        ...
        <div class="grid lg:grid-cols-3 gap-4 sm:gap-8">
            ...
            <div class="lg:col-span-2 space-y-4">
                @foreach($items as $item)
                    <div class="bg-white rounded-xl shadow-sm p-4" wire:key="cart-item-{{ $item['product']->id }}">
                        <!-- Top row: Image + Details + Remove (desktop: all inline) -->
                        <div class="flex items-start sm:items-center gap-3 sm:gap-4">
                            ...
                            <div class="flex-1 min-w-0">
                                <a href="{{ route('product.show', $item['product']->slug) }}"
                                   class="text-sm sm:text-base text-gray-900 font-semibold hover:text-purple-600 transition truncate block">
                                    {{ $item['product']->nom }}
                                </a>
                                <p class="text-xs sm:text-sm text-gray-500 mt-0.5">{{ $item['product']->category->nom ?? '' }}</p>
                                <div class="mt-1">
                                    <span class="text-base sm:text-lg font-bold bg-gradient-to-r from-purple-600 to-pink-500 bg-clip-text text-transparent">
                                        {{ number_format($item['product']->prix, 0, ',', ' ') }} Ar
                                    </span>
                                </div>
                            </div>

                            <!-- Remove Button (visible on mobile top-right, desktop inline) -->
                            <div x-data="{ confirmDelete: false }" class="flex-shrink-0 sm:hidden">
                                <template x-if="!confirmDelete">
                                    <button @click="confirmDelete = true"
                                            class="w-8 h-8 rounded-lg bg-red-50 flex items-center justify-center text-red-500 hover:bg-red-100 hover:text-red-600 transition">
                                ...
                                <template x-if="confirmDelete">
                                    <div class="flex items-center space-x-1">
                                        <button wire:click="removeItem({{ $item['product']->id }})"
                                                class="px-2 py-1 bg-red-500 text-white text-xs font-medium rounded-lg hover:bg-red-600 transition">
                                            Oui
                                        </button>
                                        <button @click="confirmDelete = false"
                                                class="px-2 py-1 bg-gray-100 text-gray-700 text-xs font-medium rounded-lg hover:bg-gray-200 transition">
                                            Non
                                        </button>
                                    </div>
                                </template>
                        ...
                        <!-- Bottom row on mobile: Quantity + Subtotal + Remove(desktop) -->
                        <div class="flex items-center justify-between mt-3 pt-3 border-t border-gray-100 sm:border-0 sm:mt-0 sm:pt-0 sm:pl-24 sm:ml-4">
                            <!-- Quantity -->
                            <div class="flex items-center">
                                <button wire:click="decrementQuantity({{ $item['product']->id }})"
                                        class="w-8 h-8 sm:w-9 sm:h-9 rounded-l-lg bg-gray-100 flex items-center justify-center text-gray-600 hover:bg-purple-100 hover:text-purple-600 transition border border-gray-200 disabled:opacity-50 disabled:cursor-not-allowed"
                                        {{ $item['quantity'] <= 1 ? 'disabled' : '' }}>
                                    <svg class="w-3.5 h-3.5 sm:w-4 sm:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 12H4"></path>
                                    </svg>
                                </button>
                                <span class="w-10 h-8 sm:w-12 sm:h-9 flex items-center justify-center font-bold text-sm sm:text-base text-gray-900 bg-white border-t border-b border-gray-200">{{ $item['quantity'] }}</span>
                                <button wire:click="incrementQuantity({{ $item['product']->id }})"
                                        class="w-8 h-8 sm:w-9 sm:h-9 rounded-r-lg bg-gray-100 flex items-center justify-center text-gray-600 hover:bg-purple-100 hover:text-purple-600 transition border border-gray-200 disabled:opacity-50 disabled:cursor-not-allowed"
                            ...
                            <div class="text-right">
                                ...
                                <p class="text-base sm:text-lg font-bold bg-gradient-to-r from-purple-600 to-pink-500 bg-clip-text text-transparent">
                                    {{ number_format($item['product']->prix * $item['quantity'], 0, ',', ' ') }} Ar
                                </p>
                            </div>

                            <!-- Remove Button (desktop only) -->
                            <div x-data="{ confirmDelete: false }" class="ml-2 flex-shrink-0 hidden sm:block">
                                <template x-if="!confirmDelete">
                                    <button @click="confirmDelete = true"
                ...
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 pt-4">
                    <a href="{{ route('shop') }}" class="text-purple-600 hover:text-purple-800 font-medium flex items-center space-x-2 text-sm sm:text-base">
            ...
            <div class="lg:col-span-1">
                <div class="bg-white rounded-xl shadow-sm p-6 sticky top-24">
                    ...
                    <a href="{{ route('checkout') }}"
                       class="mt-6 w-full bg-gradient-to-r from-purple-600 to-pink-500 text-white font-semibold py-3 px-4 sm:py-4 sm:px-8 rounded-full hover:opacity-90 transition transform hover:scale-[1.02] shadow-lg flex items-center justify-center space-x-2 text-sm sm:text-base">
